                                                  ...
                                                  MPIR_Comm * comm_ptr, MPIR_Errflag_t errflag)
{
    ...
    MPIR_CHKLMEM_MALLOC_CIVL(datatype, tmp_buf, count * (MPL_MAX(extent, true_extent)));
    ...
    /* PDH: needed to change this to use MPL_pof2 like recursive doubling
    ...
    */
    ...
    if (newrank != -1) {
        ...
        while (mask < pof2) {
            ...
            /* Need to modify because CIVL does like byte-level pointer math
            ...
            */
            mpi_errno = MPIC_Sendrecv(recvbuf +
                                      disps[send_idx],
                                      send_cnt, datatype,
                                      dst, MPIR_ALLREDUCE_TAG,
                                      tmp_buf +
            ...
            */
            mpi_errno = MPIR_Reduce_local(tmp_buf + disps[recv_idx],
                                          recvbuf + disps[recv_idx],
                                          recv_cnt, datatype, op);
            MPIR_ERR_CHECK(mpi_errno);
