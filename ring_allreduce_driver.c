...
int MPIR_Allreduce_intra_ring(const void *,
                              void *,
                              MPI_Aint,
                              MPI_Datatype,
                              MPI_Op, MPI_Comm);
...
int main(int argc, char *argv[]) {
    ...
    MPIR_Allreduce_intra_ring(x, my_allreduce_result, N, MPI_DOUBLE, AR_OP, MPI_COMM_WORLD);
...
int My_Allreduce(void *x, void *result, int bufsize, MPI_Datatype datatype, MPI_Op op, MPI_Comm comm){
   ...
   if (rank == 0) {
      ...
      memcpy(result,x, bufsize * size);  // Copy local x to result buffer
      MPI_Reduce(MPI_IN_PLACE, result, bufsize, datatype, op, 0, comm);
   } else {
